...
class SACAgent(BasicAgent):
    def __init__(self,
                 ...
                 learning_updates_per_learning_session=10,
                 batch_size=64):
        super(SACAgent, self).__init__(config, load_model, use_GPU, gamma,
        ...
        self.copy_of_target_network(
            self.critic_net_2, self.critic_target_net_2)
    ...
    def compute_critic_loss(self, states, actions, rewards, next_states, dones):
        with torch.no_grad():
            ...
            qf1_next_target = self.critic_target_net(
                torch.cat((next_states, next_actions), 1))
            qf2_next_target = self.critic_target_net_2(
                torch.cat((next_states, next_actions), 1))
            mean_qf_next_target = 0.5 * \
                (qf1_next_target + qf2_next_target) - self.alpha*next_log_pis
            next_q_value = rewards + (1.-dones) * \
                self.gamma * mean_qf_next_target
    ...
    def pick_action_by_policy(self, state, eval):
        ...
        if eval:
            _, _, action = self.get_action_info(state)
        else:
            with torch.no_grad():
    ...
    def update_critic(self, states, actions, rewards, next_states, dones):
        ...
        self.soft_update_of_target_network(
            self.critic_net, self.critic_target_net, self.hyperparameters["Critic"]["tau"])
        self.soft_update_of_target_network(
            self.critic_net_2, self.critic_target_net_2, self.hyperparameters["Critic"]["tau"])

    def compute_entropy_tuning_loss(self, log_pis):
        alpha_loss = -(self.log_alpha * (log_pis +
